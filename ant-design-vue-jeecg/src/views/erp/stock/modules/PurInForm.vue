    ...
    <div>
      <a-form-model ref="form" :model="model" :rules="validatorRules" >
        ...
        <a-tabs v-model="activeKey" @change="handleChangeTabs">
          ...
          <a-tab-pane v-if="srcVisible" tab="采购订单明细" key="srcEntryTable" :forceRender="true">
            <j-vxe-table
              keep-source
              ref="srcEntryTable"
          ...
          <template slot="tabBarExtraContent">
            <vxe-table-columns-setter v-show="activeKey==='entryTable'"
              ...
              :column-defs="entryTable.columns" :excluded-cols="disabled ? '':entryTable.exKeysWhenEdit" ignored-cols="swellQty" style="float: right;"/>
          </template>
        </a-tabs>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      addTip() {
        if (stringIsEmpty(this.model.supplierId)) return "请先选择供应商！";
        if (!stringIsEmpty(this.model.srcNo)) return "请从订单明细中选择添加！";
        return "";
      },

      addDisabled() {
        return stringIsEmpty(this.model.supplierId) || !stringIsEmpty(this.model.srcNo);
      },
    ...
    methods: {
      ...
      handleAddEntryFromSrc(){
        for(let srcRow of this.$refs.srcEntryTable.selectedRows) {
          ...
          row.settleQty = Number(srcRow.qty) - Number(srcRow.settleQty);
          row.qty = row.settleQty;
        ...
        switch (column.property) {
          ...
          case "unitId":
            ...
            if (!rate)
              ...
              values.qty = Number(values.settleQty); //假定：无涨吨
      ...
      onMaterialValuesChange(event) {
        ...
        this.handleMaterialChange(row, target, 'settleQty,price,settleAmt,tax,qty,cost');
      },
