<template>
  <j-modal
    ...
    <template slot="footer">
      ...
      <a-button v-if="disableSubmit" :disabled="isDisabledAuth('PurInSingle:print')" key="print" @click="handlePrint" style="margin-right: 48px">打印</a-button>
    ...
    <pur-in-form ref="realForm" @ok="submitCallback" :disabled="disableSubmit" :loading.sync="loading" :entryCount.sync="entryCount"/>
  </j-modal>
</template>

<script>
  import PurInForm from './PurInForm'
  ...
  export default {
    ...
    components: {PurInForm},

    data() {
      return {
        tip: '提示：采购入库单生效后自动生成采购应付单！',
      }
    },
  }
</script>
