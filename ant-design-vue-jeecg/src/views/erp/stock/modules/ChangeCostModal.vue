<template>
  <j-modal
    ...
    <template slot="footer">
      <a-button v-if="disableSubmit" :disabled="isDisabledAuth('ChangeCost:print')" key="print" @click="handlePrint" style="margin-right: 48px">打印</a-button>
    ...
    <change-cost-form ref="realForm" @ok="submitCallback" :disabled="disableSubmit" :loading.sync="loading" :entryCount.sync="entryCount"/>
  </j-modal>
</template>

<script>
  import ChangeCostForm from "./ChangeCostForm";
  import { BillModalMixin } from '../../common/mixins/bill/BillModalMixin'

  export default {
    name: 'ChangeCostModal',
    mixins: [BillModalMixin],
    components: { ChangeCostForm },
  }
</script>
