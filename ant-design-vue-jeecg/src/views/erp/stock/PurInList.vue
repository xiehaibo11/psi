<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button :disabled="isDisabledAuth('PurIn:edit')" @click="myHandleAdd" type="link" icon="plus">新增</a-button>
      <a-button :disabled="isDisabledAuth('PurIn:edit')" type="link" icon="download" @click="handleExportXls('采购入库单')">导出</a-button>
      <a-upload name="file" :showUploadList="false" :multiple="false" :headers="tokenHeader" :action="importExcelUrl" @change="handleImportExcel">
        <a-button :disabled="isDisabledAuth('PurIn:edit')" type="link" icon="import">导入</a-button>
      </a-upload>

      <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          <a-menu-item :disabled="isDisabledAuth('PurIn:edit') || !isBatchEnabled('delete')" key="1" @click="batchDel">删除</a-menu-item>
          <a-menu-item :disabled="isDisabledAuth('PurIn:execute') || !isBatchEnabled('close')" key="2" @click="batchClose">关闭</a-menu-item>
          <a-menu-item :disabled="isDisabledAuth('PurIn:execute') || !isBatchEnabled('unclose')" key="3" @click="batchUnclose">反关闭</a-menu-item>
        </a-menu>
    ...
    <div>
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          <a :disabled="!record.actions.edit || isDisabledAuth('PurIn:edit')" @click="myHandleEdit(record)">编辑</a>
          <a-divider type="vertical" />
          <a-dropdown>
            <a class="ant-dropdown-link">更多 <a-icon type="down" /></a>
            <a-menu slot="overlay">
              <a-menu-item :disabled="isDisabledAuth('PurIn:edit') || !record.actions.delete" key="1" >
                <a-popconfirm :disabled="isDisabledAuth('PurIn:edit') || !record.actions.delete" title="确定删除吗?" @confirm="() => handleDelete(record.id)">删除</a-popconfirm>
              </a-menu-item>
              <a-menu-item v-if="'check' in record.actions" :disabled="!record.actions.check || isDisabledAuth('PurIn:check')" key="2" @click="handleCheck(record)">审核</a-menu-item>
              <a-menu-item v-if="'ebpm' in record.actions" :disabled="!record.actions.ebpm || isDisabledAuth('PurIn:ebpm')" key="3" @click="handleEbpm(record)">结束审批</a-menu-item>
              <a-menu-item v-if="'execute' in record.actions" :disabled="!record.actions.execute || isDisabledAuth('PurIn:execute')" key="4" @click="handleExecute(record)">执行</a-menu-item>
              <a-menu-item v-if="'close' in record.actions" :disabled="!record.actions.close|| isDisabledAuth('PurIn:execute')" key="5">
                <a-popconfirm title="确定关闭吗?" @confirm="()=> handleClose(record.id)">关闭</a-popconfirm>
              </a-menu-item>
              <a-menu-item v-if="'unclose' in record.actions" :disabled="!record.actions.unclose || isDisabledAuth('PurIn:execute')" key="6" >
                <a-popconfirm title="确定反关闭吗?" @confirm="() => handleUnclose(record.id)">反关闭</a-popconfirm>
              </a-menu-item>
              <a-menu-item v-if="'void' in record.actions" :disabled="!record.actions.void || isDisabledAuth('PurIn:void')" key="7" @click="handleVoid(record)">作废</a-menu-item>
              <a-menu-item :disabled="isDisabledAuth('PurIn:print')" key="9" @click="handlePrint(record.id)">打印</a-menu-item>
           </a-menu>
          </a-dropdown>
        </span>
...
<script>
  ...
  import BillModal from './modules/PurInModal'
  ...
  export default {
    ...
    components: {BillModal, TableColumnsSetter},
    ...
    data () {
      return {
        ...
        url: {
          ...
          exportXlsUrl: "/stock/stkIo/exportXls/101/0/0",
          importExcelUrl: "stock/stkIo/importExcel/101/0/0",
        },
        dictOptions:{},
      }
    },
