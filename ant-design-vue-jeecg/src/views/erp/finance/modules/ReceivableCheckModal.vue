<template>
  <j-modal
    ...
    <template slot="footer">
      <a-button v-if="disableSubmit" :disabled="isDisabledAuth('ReceivableCheck:print')" key="print" @click="handlePrint" style="margin-right: 48px">打印</a-button>
      ...
      <a-tooltip :title="canSubmit ? '' : '无核销或上下合计不等，不能提交！'" placement="bottom">
        <a-button v-if="!disableSubmit" key="submit" @click="handleMySubmit" type="primary" :disabled="loading || !canSubmit">提交</a-button>
      </a-tooltip>
    ...
    <receivable-check-form ref="realForm" @ok="submitCallback" :disabled="disableSubmit" :loading.sync="loading" :canSubmit.sync="canSubmit"/>
  </j-modal>
</template>
