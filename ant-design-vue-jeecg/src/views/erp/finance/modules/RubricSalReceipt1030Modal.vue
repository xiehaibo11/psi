<template>
  <j-modal
    ...
    <template slot="footer">
      ...
      <a-button v-if="disableSubmit" :disabled="isDisabledAuth('RubricSalReceipt1030:print')" key="print" @click="handlePrint" style="margin-right: 48px">打印</a-button>
    ...
    <rubric-sal-receipt1030-form ref="realForm" @ok="submitCallback" :disabled="disableSubmit" :loading.sync="loading" :entryCount.sync="entryCount"/>
  </j-modal>
</template>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        tip: '提示：(红字)销售收款单生效后自动生成(红字)应收核销单！',
      }
    },
  }
</script>
